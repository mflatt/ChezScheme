...
static INT glzemit_lz4(lz4File_out *lz4, void *buffer, UINT count) {
  ...
  if ((cached_out_buffer = (sized_buffer *)LZ4OUTBUFFER(tc)) == NULL || cached_out_buffer->size < lz4->out_buffer_size) {
    if (cached_out_buffer != NULL) free(cached_out_buffer);
    if ((LZ4OUTBUFFER(tc) = (ptr)(cached_out_buffer = malloc(sizeof(sized_buffer) + lz4->out_buffer_size))) == 0) return -1;
