                   ...
                   `(let ([,n (inline ,(make-info-unboxed-args '(#t)) ,%fpcastto ,e)])
                      (let ([,m ,(%inline logand ,n (immediate ,(constant immediate-flonum-drop-mask)))])
                        (if ,(%inline eq? ,m (immediate ,(constant immediate-flonum-drop-mask)))
                            ,(%inline logxor
                                      ,(%inline rot ,n (immediate ,(constant immediate-flonum-offset)))
  ...
  (with-output-language (L7 Expr)
    ...
    (define build-flonum-allocation-from-mem
      (lambda (base index offset)
        (bind #f (base index)
          (constant-case immediate-flonums
            [(#t)
             (bind #t ([n `(inline ,(make-info-load 'unsigned-64 #f) ,%load ,base ,index
                                   (immediate ,offset))])
             ...
             (bind #t ([t (%constant-alloc type-flonum (constant size-flonum))])
               `(seq
                 (set! ,(%mref ,t ,(constant flonum-data-disp))
                       (inline ,(make-info-load 'unsigned-64 #f) ,%load ,base ,index
                               (immediate ,offset)))
                 ,t))]))))
    ...
    (define build-$inexactnum-imag-part
      (lambda (e)
        (constant-case immediate-flonums
          [(#t)
           (build-flonum-allocation-from-mem e %zero (constant inexactnum-imag-disp))]
          [else
                   ...
                   (bind #t (base index)
                     (bind #t ([t (%constant-alloc type-flonum (constant size-flonum))])
                       (%seq
                         ...
                         ,t)))]
                  [(64)
                   (build-flonum-allocation-from-mem base index offset)])
                (bind #f (base index)
         ...
         (case type
           ...
           [(double-float)
            `(inline ,(make-info-load 'unsigned-64 #t) ,%store
               ...
               (raw ,(%flonum-unpack-as-int ,value)))]
           ; 40-bit+ only on 64-bit machines
           [(integer-8 integer-16 integer-24 integer-32 integer-40 integer-48 integer-56 integer-64
             unsigned-8 unsigned-16 unsigned-24 unsigned-32 unsigned-40 unsigned-48 unsigned-56 unsigned-64)
            ...
            (%inline logand
              ,(let ([body (constant-case native-endianness
                             [(unknown)
                              (constant-case ptr-bits
                                [(64)
                                 (%inline srl ,(%flonum-unpack-as-int ,e1) (immediate 32))]
                                [(32)
       ...
       (bind #t (e1 e2)
         `(if ,(build-fl= e1 e1) ; check e1 not +nan.0
              ,(constant-case ptr-bits
                ...
                [(64) (%inline eq?
                        (raw ,(%flonum-unpack-as-int ,e1))
                        (raw ,(%flonum-unpack-as-int ,e2)))]
                [else ($oops 'compiler-internal
