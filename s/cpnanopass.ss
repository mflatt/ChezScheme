    ...
    (define-pass np-expand-foreign : L4.5 (ir) -> L4.75 ()
      (Expr : Expr (ir) -> Expr ()
        [(call ,info1 ,mdcl (foreign ,info ,e) ,[e*] ...)
         (if (memq 'atomic (info-foreign-conv* info))
             ;; convert atomic calls directly to `foreign-call`
             `(foreign-call ,info ,(Expr e) ,e* ...)
             ;; convert non-atomic calls to go through a function
             ;; generated by `(foreign ,info ,e)`, because during a
             ...
             ;; potential improvement: convert to `foreign-call` when in tail position
             (let ([e (Expr (with-output-language (L4.5 Expr)
                              `(foreign ,info ,e)))])
               `(call ,info1 ,mdcl ,e ,e* ...)))]
        [(foreign ,info ,[e])
